...
```
 ...
 - 타겟마케팅전략 수립
     - 2030
         - 전략 : 2030 'Small Luxury & Flex' 패키징 (Target: 20s-30s)
     ...
     - 20
         - 전략 : 20대 타겟 '스몰 럭셔리 게이트웨이' (Target: 20s)
...
### TIL (간단한 Keyword 정리)
## 트러블슈팅 (Troubleshooting)
# meat_dinner_df['ATV'] / fastfood_df['ATV']
# 10개/25개 -> 1:1매칭 불가
...
```
 ...
 - [ ] 타겟마케팅을 위해선 2030, 4050, 60+ 이상의 그룹이 소비패턴이 얼마나 다른지를 파악해야 함.
 - [ ] 특히 4050은 단순히 경제적 여유와 지위에서 나오는 '많은 자본' 때문에 높은 매출인 것인지, '만족도'에서 지갑이 열리는 것 때문인지 파악해야 함.
 - [ ] 반대로 경제적 여유와 지위가 상대적으로 덜 한 2030은 넉넉한 자본이 없이 때문에, 평소엔 가성비(패스트푸드)를 먹고, 보상심리/이벤트로써 고기요리에서 폭발적인 매출을 내는 것인지 봐야함. 
