...
"""
# 📅 개발 및 학습 일지 (Development Log)
## 22.Dec.2025 [DAY-15] (작업 환경 변경으로 인한 추후 업데이트 예정)
## ✅ 한 일
 - [x] R 카이제곰검정
 - [x] 세부 업종 분석 시작
 - [x] 작업환경 변경 준비
...
통계적 유의수준 0.05(신뢰수준 95%)는 물론 0.01 (신뢰수준 99%)보다도 훨씬 낮다.

30~60대 점심 소비: (비중: 16.70%, 이용건수: [ ]건, 매출액: [ ]원) → 점심에도 일정 수요 존재
  - 직장인 점심 회식, 가족 모임 등으로 인해 점심 시간대에도 고기 요리 소비가 일정 부분 발생

20대 점심 소비: (비중: 9.95%, 이용건수: [ ]건, 매출액: [ ]원) → 수요가 거의 없음
  - 역으로 해석하면, 20대 소비의 90.05% (주로 저녁)에 집중

두 집단 간의 소비 패턴 차이가 우연일 확률이 **0%**에 수렴한다
...
### 3. 카이제곱 검정
 ...
 - **방법 :** 
     ...
     - 매핑 및 age 수정
 - **결과 : 카이제곱 검정 결과, '20대의 점심 시간대 매출 기여도는 주 소비층에 비해 현저히 낮다'는 가설입증.**
